...
exports.updateSettings = async (req, res) => {
    try {
        ...
        res.json({ success: true, data: settings, message: 'Luu c?u hình thành công' });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};
...
exports.getPublicSettings = async (req, res) => {
    ...
    const defaults = {
        general: {
            siteName: 'A Phim',
            siteDesc: 'N?n t?ng xem phim tr?c tuy?n hàng d?u Vi?t Nam',
            siteDomain: 'APhim.vn',
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                // vnPayKey và zaloPayKey b? ?n ch? d?ng (sensitive)
            }
        });
    } catch (error) {
